...
class DistributedWeightedSampler(Sampler):
    """
    Distributed weighted sampler for multi-process training.

    Similar behavior to DistributedSampler, but draws indices according to
    per-sample weights (with replacement), then shards sampled indices by rank.
    """
    ...
    def __init__(
        ...
        if torch.is_tensor(sample_weights):
            sample_weights = sample_weights.detach().cpu().to(dtype=torch.double)
        else:
            sample_weights = torch.as_tensor(sample_weights, dtype=torch.double)

        if sample_weights.ndim != 1:
            raise ValueError("sample_weights must be 1D")
        if len(sample_weights) == 0:
            raise ValueError("sample_weights must be non-empty")
        if (sample_weights < 0).any():
            raise ValueError("sample_weights must be non-negative")
        if float(sample_weights.sum().item()) <= 0:
            raise ValueError("sum(sample_weights) must be > 0")

        self.sample_weights = sample_weights
        self.dataset_len = len(sample_weights)
...
class BaseDataset(Dataset):
    ...
    def _parse_annotations(self):
        ...
        for key, value in anno_db.items():
            ...
            fps = float(value.get('fps', 30))
            duration = float(value['duration'])
            # Derive num_frames from duration and fps (virtual frame count)
            ...
            if 'num_clips' in value:
                ...
                # Estimate num_clips from duration and feature extraction rate
                # fps / clip_stride = features per second
                clip_stride_raw = self.clip_stride // self.downsample_rate
                num_clips = max(1, int(duration * fps / clip_stride_raw))
                num_clips = (num_clips + self.downsample_rate - 1) // self.downsample_rate
    ...
    def load_feat_as_numpy(self, base_path):
        """
        Load feature from .npy or .pt file and return as a NumPy array (float32).
        """
        for ext in ['.npy', '.pt']:
        ...
        raise FileNotFoundError(f"No .npy or .pt file found for base path: {base_path}")
    ...
    def _load_vid_feats(self, vid_id):
        try:
            ...
            vid_feats = [self.load_feat_as_numpy(f) for f in vid_feat_files]
        except:
            raise ValueError(
...
class VideoCentricDataset(BaseDataset):
    ...
    def _build_train_samples(self):
        """Build training samples with progress indication.
        
        NOTE: We only store ONE epoch of samples to save memory.
        The epoch indexing in __getitem__ uses modulo to wrap around.
        """
        vid_ids = list(self.vid_dict.keys())
        
        single_epoch_samples = []
        for vid_id in tqdm(vid_ids, desc="Building train samples", disable=len(vid_ids) < 100):
            single_epoch_samples += self._group(vid_id)
        
        # Store only ONE epoch of samples to save memory
        # Previously: samples = single_epoch_samples * self.num_epochs (HUGE memory!)
        return tuple(single_epoch_samples)
    ...
    def __getitem__(self, idx):
        # Use modulo to wrap around for multi-epoch training
        # This allows us to store only one epoch of samples
        actual_idx = idx % len(self.data_list)
        vid_id, seg_idx = self.data_list[actual_idx]
        vid_dict = self.vid_dict[vid_id]
        ...
        return {
                 ...
                 'vid_id'     : vid_id,                 # video identifier
                }
...
def make_dataloader(
    ...
    sample_weights=None # optional per-sample weights for weighted sampling
):
    sampler = None
